document.addEventListener('DOMContentLoaded', () => {
    ...
    // إغلاق عند النقر خارج النافذة المربعة (على الخلفية الضبابية)
    ...
    searchButton.addEventListener('click', async () => {
        ...
        try {
            ...
            if (result.status === 200) {
                ...
                showError(`خطأ من الخادم: ${result.message}`);
            }

        } catch (error) {
            console.error("Error:", error);
            showError("فشل الاتصال بالخادم. يرجى التأكد من تشغيل ملف api.php بشكل صحيح.");
        } finally {
